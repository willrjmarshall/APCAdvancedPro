...
MASTER_SCALE_MAX = 0.90
MASTER_SCALE_MIN = 0.50
  ...
  def setup_matrix(self, vu_set, master):
    ...
    if master:
      for scene in self.parent._parent._session._scenes:
        matrix.append(scene._disabled_launch_button)
    else:
      for index, column_index in enumerate(vu_set):
        matrix.append([])
        column = matrix[index]
        strip = self.parent._parent._mixer.channel_strip(column_index)
        column.extend([strip._arm_button, strip._solo_button, strip._mute_button, strip._select_button])
        column.append(self.parent._parent.stop_buttons[column_index])

        for row_index in range(CLIP_GRID_Y):
          column.append(self.parent._parent._button_rows[CLIP_GRID_Y - 1 - row_index][column_index])
    return matrix
...
class VUMeters(ControlSurfaceComponent):
    ...
    def set_master_leds(self, level):
        for scene_index in range(CLIP_GRID_Y):
            ...
            if scene_index >= (CLIP_GRID_Y - level):
              scene._disabled_launch_button.send_value(LED_ON, True)
            else:
              scene._disabled_launch_button.send_value(LED_OFF, True)
    ...
    def set_leds(self, matrix, level):
        column = matrix[0] # We assume only one column for this script
